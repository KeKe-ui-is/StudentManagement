 ...
 *EC2_HOST について
  現在ドメインの取得を行っていないためEC2を起動するたびにＩＰアドレスが変更されるのでその都度変更して運用します

Secretの登録手順（Repository → Settings → Secrets の場所）
Repository secrets に登録する項目
| Secret名 | 用途 |
|---|---|
| AWS_EC2_PRIVATE_KEY | sshでEC2に接続するための鍵 |
 *ワークフローで参照するときの名前 : secrets.AWS_EC2_PRIVATE_KEY
